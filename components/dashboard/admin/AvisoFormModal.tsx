...
/**
 * AvisoFormModal — Create / Edit modal for avisos (Step 7.8).
 *
 * Image flow identical to BeneficioFormModal:
...
import type { AvisoRow, EmpresaOption } from "./AvisosAdmin";
...
interface Props {
  ...
  createdBy: string;
  onClose:   () => void;
  onSaved:   (saved: AvisoRow, isNew: boolean) => void;
}
...
export default function AvisoFormModal({
  open, mode, aviso, empresas, createdBy, onClose, onSaved,
}: Props) {
  const t = useTranslations("Dashboard.admin.sistema.avisos.modal");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let finalImageUrl = aviso?.aviso_image_url ?? null;

      // Delete existing image if marked for removal
      if (removeExisting && aviso?.aviso_image_url) {
        const path = extractStoragePath(aviso.aviso_image_url);
      ...
      if (imageFile) {
        ...
        if (mode === "editar" && aviso?.aviso_image_url && !removeExisting) {
          const oldPath = extractStoragePath(aviso.aviso_image_url);
      ...
      if (mode === "crear") {
        ...
        onSaved(data as AvisoRow, false);
      }
    } catch {
  ...
  const showExisting = mode === "editar" && !!aviso?.aviso_image_url && !removeExisting && !imageFile;
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="space-y-4">
                ...
                <FormField label={t("fieldEstado")}>
                  <select
                    value={form.estado_aviso}
                    onChange={(e) => setForm({ ...form, estado_aviso: e.target.value as "activa" | "expirada" })}
                    className={inputCls}
                  >
              ...
              <div className="space-y-4">
                    ...
                    <div className="px-3 py-1.5 bg-gray-50 border-t border-gray-100">
                      <p className="text-[10px] text-gray-400 font-roboto">
                        {form.empresa_ids.length === 0
                          ? t("empresasGlobal")
                          : t("empresasSeleccionadas", { count: form.empresa_ids.length })}
                      </p>
                    </div>
                  </div>
                </FormField>
                ...
                <FormField label={t("fieldImagen")}>
                  ...
                  {showExisting && aviso?.aviso_image_url && (
                    <div className="relative">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 shrink-0 bg-gray-50/60">
            <button
              ...
              onClick={onClose}
              disabled={saving}
              className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-roboto font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50 transition-colors"
            >
              {t("cancelarBtn")}
            </button>
            <button
              type="submit"
              disabled={saving}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-secondary text-white text-sm font-roboto font-semibold hover:bg-secondary/90 disabled:opacity-50 transition-colors"
            >
