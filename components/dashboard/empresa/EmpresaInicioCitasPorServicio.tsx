...
/**
 * EmpresaInicioCitasPorServicio — Full-width bar chart card (Step 6 - new feature).
 *
 * Shows count of citas per service for the selected month.
 * RLS citas_empresa_admin_read scopes to empresa automatically.
 *
 * - Month selector (prev / next navigation, max = current month)
 * - Horizontal bar chart for readability with long service names
 * - Independent loading skeleton, error, and empty states
 */
...
/** Capitalise first letter of a month name */
function formatMonth(year: number, month: number): string {
  return new Date(year, month, 1).toLocaleString("es-NI", {
    month: "long",
    year:  "numeric",
...
export default function EmpresaInicioCitasPorServicio() {
  const t = useTranslations("Dashboard.empresa.inicio.graficoCitas");
  ...
  // ── Dynamic chart height (min 180px, 44px per bar) ───────────────────────
