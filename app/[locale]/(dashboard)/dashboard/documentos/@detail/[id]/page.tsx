...
import DocumentoActions from "@/app/[locale]/(dashboard)/dashboard/admin/documentos/@detail/[id]/DocumentoActions";
...
export default async function DocumentoMiembroDetallePage({ params }: PageProps) {
  const { locale, id } = await params;
  const supabase = await createClient();
  const t  = await getTranslations("Dashboard.miembro.documentos.detalle");
  const tT = await getTranslations("Dashboard.miembro.documentos");
  ...
  const { data: doc, error } = await supabase
    ...
    .select("id, nombre_documento, tipo_documento, fecha_documento, file_path, tipo_archivo, created_at")
    .eq("id", id)
    .eq("usuario_id", user.id)          // RLS-level ownership check
    .eq("estado_archivo", "activo")
    .maybeSingle();

  if (error) redirect(`/${locale}/dashboard/documentos`);
  if (!doc) notFound();
  ...
  return (
    <>
      <BackButton href={`/${locale}/dashboard/documentos`} />
      <DetailPanel
        title={doc.nombre_documento ?? t("untitled")}
        closeHref={`/${locale}/dashboard/documentos`}
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
            ...
            <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-full", tipoBadge)}>
              {tT(`typeLabels.${doc.tipo_documento}` as Parameters<typeof tT>[0])}
            </span>
          </div>
