...
/**
 * BeneficioForm — Shared create/edit form for beneficios.
 *
 * Image upload flow (preserved from BeneficioFormModal):
 *  1. User selects file → client-side preview.
 *  2. On submit, uploads to beneficios-imagenes BEFORE calling the server action.
...
interface Props {
  action: (state: BeneficioFormState, formData: FormData) => Promise<BeneficioFormState>;
...
export default function BeneficioForm({ action, initial, empresas, locale, mode }: Props) {
  const t = useTranslations("Dashboard.admin.beneficios.form");
  const [isPending, startTransition] = useTransition();

  // ── Form state ─────────────────────────────────────────────────────────────
  const [titulo,              setTitulo]              = useState(initial.titulo);
  const [descripcion,         setDescripcion]         = useState(initial.descripcion);
  const [tipoBeneficio,       setTipoBeneficio]       = useState<"descuento" | "promocion">(initial.tipo_beneficio);
  const [estadoBeneficio,     setEstadoBeneficio]     = useState<"activa" | "expirada">(initial.estado_beneficio);
  ...
  const [porcentajeDescuento, setPorcentajeDescuento] = useState(initial.porcentaje_descuento);
  ...
  const showExisting = mode === "editar" && !!initial.beneficio_image_url && !removeExisting && !imageFile;
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      let finalImageUrl = initial.beneficio_image_url ?? null;

      // Delete existing image if marked for removal
      if (removeExisting && initial.beneficio_image_url) {
        const path = extractStoragePath(initial.beneficio_image_url);
      ...
      if (imageFile) {
        ...
        if (mode === "editar" && initial.beneficio_image_url && !removeExisting) {
          const oldPath = extractStoragePath(initial.beneficio_image_url);
      ...
      formData.set("tipo_beneficio", tipoBeneficio);
      formData.set("estado_beneficio", estadoBeneficio);
      formData.set("fecha_inicio", fechaInicio);
      formData.set("fecha_fin", fechaFin);
      formData.set("porcentaje_descuento", tipoBeneficio === "descuento" ? porcentajeDescuento.trim() : "");
      if (finalImageUrl) {
        formData.set("beneficio_image_url", finalImageUrl);
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label={t("fieldTipo")}>
              <select
                ...
                onChange={(e) => {
                  ...
                  setTipoBeneficio(newTipo);
                  if (newTipo === "promocion") setPorcentajeDescuento("");
                }}
                className={inputCls}
              >
            ...
            <Field label={t("fieldEstado")}>
              <select
                ...
                onChange={(e) => setEstadoBeneficio(e.target.value as "activa" | "expirada")}
                className={inputCls}
              >
          ...
          {/* Porcentaje — only for descuento */}
          {tipoBeneficio === "descuento" && (
            <Field label={t("fieldPorcentaje")}>
              <input
                type="text"
                maxLength={20}
        ...
        <div className="space-y-4">
          ...
          <Field label={t("fieldImagen")} error={errors.imagen}>
            ...
            {showExisting && initial.beneficio_image_url && (
              <div className="relative">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={initial.beneficio_image_url}
                  alt={t("imagenActual")}
                  className="w-full h-28 object-cover rounded-xl border border-gray-200"
                />
                <button
